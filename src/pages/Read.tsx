...
export default function Read() {
  const readingActivities = activitiesData.filter(a => a.skillType === 'reading');
  const [currentIndex, setCurrentIndex] = useState(0);
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null);
  const [isCorrect, setIsCorrect] = useState<boolean | null>(null);
  const { addReadingScore, markReadingCompleted } = useAppStore();
  const navigate = useNavigate();

  const currentActivity = readingActivities[currentIndex];

  if (!currentActivity) return <div className="p-6">Tiada latihan membaca.</div>;

  // Custom audio integration
  const activityKeys = readingActivities.map(act => `audio_activity_${act.id}`);
  ...
  const handleSelect = (choice: string) => {
    ...
    if (correct) {
      addReadingScore(10);
      markReadingCompleted(currentActivity.id);
    }
  };
  ...
  const handleNext = () => {
    ...
    setIsCorrect(null);
    if (currentIndex < readingActivities.length - 1) {
    ...
    <div className="flex flex-col h-full relative">
      <div className="px-6 py-6 sticky top-0 z-10 backdrop-blur-sm bg-white/70 border-b border-sky-100">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-xl font-bold text-navy">Latihan Membaca</h1>
        </div>
        ...
        <div className="w-full bg-sky h-2.5 rounded-full mt-4 overflow-hidden">
          <div 
            ...
            style={{ width: `${((currentIndex) / readingActivities.length) * 100}%` }}
          />
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col items-center">
        ...
        <div className="bg-white border-2 border-purple/30 p-8 rounded-[32px] shadow-lg shadow-purple/10 w-full max-w-sm flex flex-col items-center justify-center mb-8 min-h-[200px] relative overflow-hidden">
           <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-purple/10 to-transparent rounded-bl-full"></div>
           
           <button 
             onClick={handlePlayAudio}
             className={`absolute top-4 right-4 text-purple p-2.5 rounded-full hover:bg-purple/20 active:scale-90 transition-all ${
               isPlaying ? 'bg-purple/30 animate-pulse scale-110' : 'bg-purple/10'
             }`}
           >
             <Volume2 className="w-5 h-5" />
           </button>
           
           <div className="font-arabic text-7xl text-navy leading-relaxed text-center relative z-10" dir="rtl">{currentActivity.arabicText}</div>
        </div>

        <div className="w-full max-w-sm grid grid-cols-2 gap-4">
          {currentActivity.choices.map((choice, idx) => {
            let btnClass = "bg-white border-2 border-sky text-navy font-bold text-base shadow-sm";
            ...
            return (
              <button
                ...
                className={`py-5 px-4 rounded-[24px] transition-all w-full text-center font-arabic text-3xl ${btnClass} ${
            ...
            >
              <div className={`p-4 rounded-2xl flex items-center gap-3 mb-4 border-2 ${isCorrect ? 'bg-green/10 border-green/30 text-green' : 'bg-pink/10 border-pink/30 text-pink'}`}>
                ...
                <span className="font-bold text-[15px]">
                  {isCorrect ? 'Bagus, kamu dapat membaca perkataan ini!' : 'Cuba lagi. Perhatikan huruf pada perkataan.'}
                </span>
              </div>
              <button 
                onClick={handleNext}
