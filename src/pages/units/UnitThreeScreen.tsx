...
import { ColorVocabularyCard } from "../../components/units/ColorVocabularyCard";
...
export default function UnitThreeScreen() {
  const navigate = useNavigate();
  const { vocabulary, loading } = useUnitVocabulary("unit_3_colors");
  const { assets, loading: assetsLoading } = useAppAssets(["muallimah_ummi_avatar"]);
  const { videoUrl, loading: videoLoading, error: videoError } = useUnitVideo("unit_3_video");
  ...
  const playAudio = (item: UnitVocabulary) => {
    if (item.audioUrl) {
  ...
  const playAudioFromEvent = (e: React.MouseEvent, item: UnitVocabulary) => {
    e.stopPropagation();
    playAudio(item);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 relative overflow-hidden flex flex-col">
      ...
      <UnitHeader
        unitLabel="الوحدة الثالثة"
        titleArabic="الألوان حولنا"
        titleMalay="Warna"
        themeColor="blue"
      />
      ...
      <div className="flex-1 px-4 pt-12 pb-8 z-10 max-w-4xl mx-auto w-full">
        <UnitGuideBubble
          ...
          message="Dengar dan baca nama warna dalam Bahasa Arab."
          themeColor="blue"
        />
          ...
          <div className="flex flex-wrap justify-between gap-y-4">
            {vocabulary.map((item) => (
              <ColorVocabularyCard
                key={item.id}
                item={item}
                openVocabularyDetail={openVocabularyDetail}
                playAudio={playAudio}
              />
            ))}
          </div>
        ...
        <div className="mt-10 flex flex-col gap-3 max-w-sm mx-auto">
          ...
          <button
            ...
            className="w-full bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-4 px-6 rounded-2xl shadow-lg shadow-indigo-500/30 flex items-center justify-center gap-3 transition-transform active:scale-95"
          >
      ...
      <VocabularyDetailModal
        ...
        onPlayAudio={playAudioFromEvent}
        themeColor="blue"
      />
    </div>
  );
